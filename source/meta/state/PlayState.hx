...
import gameObjects.*;
import gameObjects.songScripts.*;
...
class PlayState extends MusicBeatState
{
	...
	public static var songScript:Dynamic;
	...
	public static var lerpVal:Float;
	...
	{
	case 'phantasm':
		songScript = new Phantasm(this);
		add(songScript);
	case 'becoming-uncanny':
		songScript = new BecomingUncanny(this);
		add(songScript);
	case 'uncany':
		songScript = new Uncany(this);
		add(songScript);
	}
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (!inCutscene)
		{
			...
			lerpVal = (elapsed * 2.4) * cameraSpeed;
	...
	private function callDefaultSongEnd()
	{
		...
		difficulty = difficulty.replace('-uncanny', '');
