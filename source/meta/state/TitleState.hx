...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		UNCANNY = new FlxSprite(FlxG.width * 0.4, FlxG.height * 0.07 - 400).loadGraphic(Paths.image("menus/base/title/3"));
		UNCANNY.setGraphicSize(Std.int(UNCANNY.width * 0.3));
		add(UNCANNY);
		add(logoBl);
	...
	if (UNCANNY != null)
		{
			uncannyStretch += elapsed;
			UNCANNY.angle += (Math.sin(uncannyStretch * 2.5) + 1) * 3.5;
		}
